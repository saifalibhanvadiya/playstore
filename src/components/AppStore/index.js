...
class AppStore extends Component {
  state = {filtersearch: '', activeIdtab: tabsList[0].tabId}

  getfilterapps = () => {
    const {activeIdtab} = this.state
    const filteredapps = appsList.filter(each => each.category === activeIdtab)
    return filteredapps
  }

  updatedapps = tabId => {
    this.setState({activeIdtab: tabId})
  }
  ...
  render() {
    ...
    const {activeIdtab, filtersearch} = this.state

    const searchFiltedData = filteredapps.filter(each =>
      each.appName.toLowerCase().includes(filtersearch.toLowerCase()),
    )
        ...
        <ul className="tabbuttons">
          {tabsList.map(each => (
            <TabItem
              tabsitems={each}
              key={each.tabId}
              updatedapps={this.updatedapps}
              isActive={each.tabId === activeIdtab}
            />
          ))}
        </ul>

        <ul className="appboxcontainer">
          {searchFiltedData.map(each => (
